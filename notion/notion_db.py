...
import requests
import notion_utils
from notion_utils import notion_request_decorator
...
class NotionDatabase:
    ...
    def _get_database_rows(self, exclude_rows_ids: list | tuple = ()) -> dict:
        ...
        for row in data:
            ...
            columns = []  # TODO maybe better to use dict
            for prop_name, prop_data in row['properties'].items():
                parser = self.NOTION_PARSERS[prop_data['type']]
                columns.append(
                    parser(prop_name, prop_data)
                )
